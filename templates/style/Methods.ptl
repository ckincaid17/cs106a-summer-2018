<dl>
	...
	<dd>
		<p>Each method should perform a single, clear, coherent task.  No one method should do too large a share of the overall work.  If you have a single method that is very long, break it apart into smaller sub-methods. As a rough estimate, a method whose body (excluding the header and closing brace) has more than 20 lines is too long.  If you try to describe the method's purpose and find yourself using the word "and" a lot, that probably means the method does too many things and should be split into sub-methods.</p>
	</dd>
...
public void moveToWall() {
	<em>while (frontIsClear()) {</em>
		...
		<div class="row">
			<div class="col-xs-12 col-sm-6">
				<pre class="code-bad">
<span class="code-comment">// bad (mutual recursion)</span>
public void methodOne() {
	if (frontIsClear()) {
		move();
		<em>methodTwo();</em>
